...
BER = zeros(4, 2, length(SNR_dB_arr));
...
for ch_idx = 1:2
    ...
    for snr_idx = 1:length(SNR_dB_arr)
        dB = SNR_dB_arr(snr_idx);
        SNR = 10^(dB/10);
        fprintf('Channel L=%d, SNR = %d dB\n', L, dB);
        ...
        if ~skip_flags(1)
            ...
            BER(1, ch_idx, snr_idx) = err1 / length(Bits1);
            if err1 == 0
        ...
        if ~skip_flags(2)
            ...
            BER(2, ch_idx, snr_idx) = err2 / length(Bits2);
            if err2 == 0
        ...
        if ~skip_flags(3)
            ...
            BER(3, ch_idx, snr_idx) = err3 / numel(Bits3);
            if err3 == 0
        ...
        if ~skip_flags(4)
            ...
            BER(4, ch_idx, snr_idx) = err4 / numel(Bits4);
            if err4 == 0
...
for ch_idx = 1:2
    ...
    for sys = 1:4
        ...
        ber_data = squeeze(BER(sys, ch_idx, :));
        
        if ch_idx == 1
            semilogy(SNR_dB_arr, ber_data, style, ...
