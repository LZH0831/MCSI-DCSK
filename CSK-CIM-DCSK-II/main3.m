...
for l_idx = 1:num_L
    ...
    for b_idx = 1:num_beta
        ...
        for snr_idx = 1:num_snr
            ...
            total_BER(l_idx, b_idx, snr_idx) =sum(Bits ~= Bitsre) / (bits * Block_Num);
...
ylim([1e-5 1]); 
xlim([0 max(SNR_dB)]);
set(gca, 'XTick', 0:2:max(SNR_dB));
xlabel('E_b/N_0 (dB)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('BER', 'FontSize', 12, 'FontWeight', 'bold');
title(sprintf('AWGN与Rayleigh信道下 \\beta 对性能的影响 (CSK-CIM-DCSK-II, m_c=%d, m_s=%d)', m_c, m_s), 'FontSize', 12);
